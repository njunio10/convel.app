    ...
    <section className="flex flex-col items-center justify-center">
      ...
      <div className="mx-auto mt-12 w-full max-w-xl">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex flex-col border-b border-brand-dark/15 bg-white"
          >
            <h3
            ...
            >
              {item.question}
              {isOpen === index ? (
                <MinusIcon className="size-5 text-brand-dark/70" />
              ) : (
                <PlusIcon className="size-5 text-brand-dark/70" />
              )}
            </h3>
            <p
              className={`pb-4 text-sm/6 text-brand-dark/70 ${
